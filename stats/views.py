...
def parse_player(steam_id):
    player_heroes = requests.get(
        'https://api.opendota.com/api/players/' + str(steam_id) + '/heroes?date=180&lobby_type=7')
    hero_info = requests.get('https://api.opendota.com/api/heroStats')
    player = requests.get(
        'https://api.opendota.com/api/players/' + str(steam_id))
    ...
    for i in parsed_player_heroes:
        ...
        hero_wins.append(
            (player_winrate, namelist[int(hero_id)], compare_winrate))
        comparedict[hero_id] = compare_winrate
        top_compares.append(
            (compare_winrate, namelist[int(hero_id)], i['games']))
    ...
    filtered_compares = list(filter(lambda x: x[2] > 5, top_compares))
    filtered_compares.sort(key=lambda x: x[0], reverse=True)

    return persona_name, solo_mmr, filtered_compares


def results(request):
    #print request.POST['steam_id']
    #steam_id = request.POST['steam_id']
    steam_id1, steam_id2, steam_id3, steam_id4, steam_id5 = request.POST['steam_id1'], request.POST[
        'steam_id2'], request.POST['steam_id3'], request.POST['steam_id4'], request.POST['steam_id5']

    data1 = parse_player(steam_id1)
    data2 = parse_player(steam_id2)
    data3 = parse_player(steam_id3)
    data4 = parse_player(steam_id4)
    data5 = parse_player(steam_id5)

    # return render(request, 'stats/results.html', {'player':player,
    # 'solo_mmr': solo_mmr, 'name':name, 'persona_name':persona_name,
    # 'top3':hero_wins[:3], 'top3_relative': filtered_compares[:5]})
    return render(request, 'stats/results.html', {'data1': data1, 'data2': data2, 'data3': data3, 'data4': data4, 'data5': data5})
...
def lookup(request, steam_id):
    player = requests.get(
        'https://api.opendota.com/api/players/' + str(steam_id))
    heroes = requests.get(
        'https://api.opendota.com/api/players/' + str(steam_id) + '/heroes')
